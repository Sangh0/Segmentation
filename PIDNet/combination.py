...
class Combination(nn.Module):
    ...
    def forward(self, inputs, labels, edges):
        ...
        pix_acc = self.get_pixel_acc(outputs[1], labels)
        mean_iou = self.get_mean_iou(outputs[1], labels)
        semantic_loss = self.sem_loss(outputs[:2], labels)
        boundary_loss = self.bd_loss(outputs[2], edges)
        
        filler = torch.ones_like(labels) * self.ignore_index
        bd_labels = torch.where(F.sigmoid(outputs[2][:,0,:,:])>0.8, labels, filler)
